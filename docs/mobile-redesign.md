...
- **2026-09-08** — `mobile-redesign-p2` implemented. Added `/items/pending` admin page, `Pending Items` nav entry, `getPendingItems`/`getItemByUpc`/`approveItem`/`rejectItem` API helpers, `Item.status`/`submittedByMe` web types, and Jest coverage. Verified `npx tsc --noEmit`, `npm run lint`, `npm test`, and `go build ./...`.
- **2026-09-08** — `mobile-redesign-p3` implemented and verified. Added `AuthService`, `AuthGate`, `LoginScreen`, dashboard, grocery lists/detail, pantry, bottom-nav shell, `graphql_config` with `AuthLink`/`ErrorLink`, Android/iOS platform config, and Flutter widget tests. Verified `flutter pub get`, `flutter analyze`, `flutter test`.
- **2026-09-08** — `mobile-redesign-p4` implemented and verified. Added `mobile_scanner` scan screen, UPC normalization (12/13/14 digits), `itemByUpc` lookup, found-item add/remove pantry via `incrementUserItem`, unknown-barcode `submitItem` with optional `setItemNutrients`, `upc_utils` unit tests, and `Flutter` platform config. Verified `flutter analyze` and `flutter test`.
- **2026-09-08** — `mobile-redesign-p5` implemented. Added `mobile` CI job to `.github/workflows/test.yml`, updated `clients/mobile/README.md` with Google client-id configuration, updated `AGENTS.md` named-phase convention note, marked `docs/mobil-front-end.md` as superseded, and added mobile-redesign additions to `docs/graphql-schema.md` and `docs/postgres-data-model.md`.
